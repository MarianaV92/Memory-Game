// Contenu dynamique pour chaque "page" pourquoi j'ai fait ça :(

import { initConnexionPage } from "./js/connexion.js";
import { initInscriptionPage } from "./js/inscription.js";
import { initProfilPage } from "./js/profil.js";
...
const content = {
  ...
  jouer: `
<main id="cards" role="region" aria-labelledby="memory game">
  <header>
    <h1>MEMORY Game</h1>
  </header>

  <div id="selectorsContainer">
              <select id="themeSelector" aria-label="Choisissez un thème de jeu">
                    ...
                    <select id="memorySize2" name="memorySize2" aria-label="Choisissez la taille de la grille de jeu">
                        <option value="choix">Choix</option>
                ...
                <div id="messageWin" aria-live="assertive"></div>
</main>
  `,

  connexion: ` <div id="container-Connexion">
        <!-- zone de connexion -->
        <form action="" method="" id ="connexion" aria-labelledby="formConnexion">
            <h1>Connexion</h1>
  ...
  profil: `
      <div id="container-profil">
            <h1>Profil</h1>
            <form id="profilForm" aria-labelledby="formProfil" >
                    <label for="username">Nom d'utilisateur:</label>
                    <input type="text" id="username" name="username" placeholder="Nom d'utilisateur" required />
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" placeholder="Votre email" required />
                    <label for="level">Choix du Memory:</label>
                  <select id="themeSelector2" aria-label="Sélectionner le thème">
                    <option value="choix">Choix</option>
                    <option value="extraterrestre">Extraterrestre</option>
                    <option value="animaux">Animaux</option>
                  ...
                  <div id="imageContainer2">
                    ...
                    <select id="memorySize" name="memorySize" aria-label="Sélectionner la taille de la grille">
                        <option value="choix">Choix</option>
...
function loadPage(page) {
  ...
  if (page === "jouer") {
    ...
    fetch("library.json")
      ...
      .then((library) => {
        ...
        // ____________Récupérer la taille de la grille choisie par l'utilisateur________
        ...
        //______ Afficher le tableau de jeu lors de la première exécution_________
        gameBoard.style.visibility = "visible";
        gameBoard.style.opacity = 1; // Appliquer l'animation de visibilité
        // _________Redémarrer le jeu lorsqu'on appuie sur "Espace"________
  ...
  sessionStorage.setItem("lastVisitedPage", page); // Enregistre la dernière page
}

//--------------------------------------------- Gestion DOM--------
//Navigation dynamique : écouter les clics sur les liens
// je vais selectionner tous les <a> à l'interieur de la balise nav (ici je vais chopper les liens cliqués ) forEach pour iterer sur tous les elements 
document.querySelectorAll("nav a").forEach((link) => {
  ...
  link.addEventListener("click", function (e) {
    ...
    const page = e.target.dataset.page || e.target.getAttribute("href").substring(1); // La méthode .substring(1) enlève le premier caractère de l'URL (#),
    loadPage(page)
  });
});
